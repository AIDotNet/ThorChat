...
public class ChatService
{
        ...
        ChatCompletionCreateRequest completionCreateRequest)
    {
        ...
        if (provider.Equals("openai", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                Address = !string.IsNullOrWhiteSpace(payload.Endpoint) ? payload.Endpoint : ThorOptions.OPENAI_PROXY_URL,
                Key = !string.IsNullOrWhiteSpace(payload.ApiKey) ? payload.ApiKey : ThorOptions.OPENAI_API_KEY
            };
        }
        else if (provider.Equals("azure", StringComparison.OrdinalIgnoreCase))
        {
            ...
            chatOptions = new ChatOptions()
            {
                Other = !string.IsNullOrWhiteSpace(payload.AzureApiVersion) ? payload.AzureApiVersion : ThorOptions.AZURE_OPENAI_API_VERSION,
                Address = !string.IsNullOrWhiteSpace(payload.Endpoint) ? payload.Endpoint : ThorOptions.AZURE_OPENAI_PROXY_URL,
                Key = !string.IsNullOrWhiteSpace(payload.ApiKey) ? payload.ApiKey : ThorOptions.AZURE_OPENAI_API_KEY
            };
        }
        else
        {
